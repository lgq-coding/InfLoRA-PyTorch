...
## 3、复现内容以及复现的心路历程
复现流程与总结：
首先更改由于版本因素而导致的报错，保证pytorch和torchvision使用一个较新的版本，而timm使用一个较旧的版本，以避免报错。然后对于numpy形式的向量和tensor的混用进行了修复，统一采取torch版本的，包括矩阵乘法和奇异值分解等方法。由此整体没有很大的错误，但之后在训练到task2的时候，由于索引越界报错，推断是因为没有及时创建LoRA导致，然后借助ai试着进行修复，之后训练过程可以正常进行，但是训练得到的准确率从task2开始急剧下降，且此后的变化趋势明显改变。推断可能是因为没有正确的构建矩阵B，或者是直接利用了空白的LoRA块，之后将结合具体的源码进行理解和修复。
## 4、尝试复现本项目
## 5、实验结果与分析
## 6、进一步思考
